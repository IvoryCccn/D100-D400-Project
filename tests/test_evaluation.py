...
def test_find_optimal_threshold_supports_youden_metric():
    y_true = pd.Series([0, 0, 0, 1, 1, 1])
    y_proba = np.array([0.05, 0.10, 0.20, 0.70, 0.80, 0.90])

    best_t, table = find_optimal_threshold(y_true=y_true, y_proba=y_proba, metric="youden")

    assert 0.0 <= best_t <= 1.0
    assert isinstance(table, pd.DataFrame)
    assert "threshold" in table.columns
    assert "youden" in table.columns
    assert "f2" in table.columns
    assert "tp" in table.columns and "fp" in table.columns and "tn" in table.columns and "fn" in table.columns
